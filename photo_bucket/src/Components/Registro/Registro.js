...
function Registro() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch('http://localhost:8000/subir_perfil', {
        method: 'POST',
    ...
    <div className="register-container">
      <div className="register-card">
        ...
        <div className="p-field">
          ...
          <InputText id="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Ingresa tu correo electrónico" />
        </div>
        <div className="p-field">
        ...
        <div className="p-field">
          ...
          <Password id="confirmPassword" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repite tu contraseña" feedback={false} />
        </div>
        <div className="p-field">
          <label htmlFor="profileImage">Imagen de Perfil</label>
          <FileUpload name="profileImage" accept="image/*" maxFileSize={1000000} customUpload={true} auto onUpload={onProfileImageUpload} />
        </div>
